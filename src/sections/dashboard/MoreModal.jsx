import { BookBookmark, GearSix, Pulse, UserSwitch, WarningCircle, X } from "@phosphor-icons/react";
  ...
  return (
    <section className="relative rounded-lg w-[270px] flex flex-col bg-modal-bg ">
      <section>
        ...
        <div className="mx-[8px] my-2 hover:bg-hover-primary p-4 text-text-primary font-medium text-sm rounded-lg cursor-pointer transition duration-300 ">
            Log out
        </div>
      </section>
      <button onClick={()=>setModal(false)} className="absolute top-3 right-3 text-text-primary hover:bg-red-600 rounded-full p-[0.10rem]" ><X size={14} /></button>
    </section>
  );
};
